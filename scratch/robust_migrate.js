...
async function run() {
  ...
  console.log('Downloading and then uploading files...');
  ...
  for (const file of files) {
    ...
    const localPath = path.join(tempDir, file.name.replace(/\//g, '_'));
    
    try {
        await file.download({ destination: localPath });
        await targetBucket.upload(localPath, { destination: file.name });
