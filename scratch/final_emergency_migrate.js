...
async function migrate() {
  console.log('Final attempt to migrate storage...');
  try {
    const [files] = await sourceBucket.getFiles();
    console.log(`Copying ${files.length} files...`);
    for (const file of files) {
      console.log(` - ${file.name}`);
      await file.copy(targetBucket.file(file.name));
    }
    console.log('SUCCESS');
  } catch (e) {
    console.log(`ERROR: ${e.message}`);
    // If target bucket failed, try appspot.com
    console.log('Retrying with target=appspot.com');
    const targetApp2 = admin.initializeApp({
      credential: admin.credential.cert(targetSa),
      storageBucket: 'freestyle-tango-seoul.appspot.com'
    }, 'target2');
    const targetBucket2 = targetApp2.storage().bucket();
    const [files] = await sourceBucket.getFiles();
    for (const file of files) {
       await file.copy(targetBucket2.file(file.name));
    }
    console.log('SUCCESS (Appspot)');
  }
}
