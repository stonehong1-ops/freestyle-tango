...
async function inspectClasses() {
  console.log("--- Inspecting tango_classes ---");
  const snap = await db.collection('tango_classes').get();
  snap.forEach(doc => {
    console.log(`ID: ${doc.id}, Data:`, JSON.stringify(doc.data()));
  });
}

inspectClasses().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
