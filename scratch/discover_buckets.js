...
async function discover(sa, name) {
  const app = admin.initializeApp({
    credential: admin.credential.cert(sa)
  }, name);
  
  try {
    const [buckets] = await app.storage().getBuckets();
    console.log(`Buckets for ${name}:`);
    buckets.forEach(b => console.log(' - ' + b.name));
    return buckets[0]?.name;
  } catch (e) {
    console.log(`Failed to list buckets for ${name}: ${e.message}`);
    return null;
  }
}
...
async function run() {
  const sourceBucket = await discover(sourceSa, 'source');
  const targetBucket = await discover(targetSa, 'target');
  console.log(`Final Discovery: Source=${sourceBucket}, Target=${targetBucket}`);
}
