const admin = require('firebase-admin');

async function checkBucket(sa, bucketName, label) {
  const app = admin.initializeApp({
    credential: admin.credential.cert(sa),
    storageBucket: bucketName
  }, label + Math.random());
  
  try {
    const bucket = app.storage().bucket();
    const [exists] = await bucket.exists();
    if (exists) {
      console.log(`✅ Bucket ${bucketName} exists for ${label}`);
      return bucketName;
    }
  } catch (e) {
    // console.log(`❌ Bucket ${bucketName} does not exist for ${label}`);
  }
  return null;
}

const sourceSa = {
  type: "service_account",
  project_id: "tangostay-7355e",
  private_key: "-----BEGIN PRIVATE KEY-----\\ \\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@tangostay-7355e.iam.gserviceaccount.com"
};

const targetSa = {
  type: "service_account",
  project_id: "freestyle-tango-seoul",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@freestyle-tango-seoul.iam.gserviceaccount.com"
};

async function run() {
  const sourceBuckets = ['tangostay-7355e.appspot.com', 'tangostay-7355e.firebasestorage.app'];
  const targetBuckets = ['freestyle-tango-seoul.appspot.com', 'freestyle-tango-seoul.firebasestorage.app'];
  
  for (const b of sourceBuckets) await checkBucket(sourceSa, b, 'Source');
  for (const b of targetBuckets) await checkBucket(targetSa, b, 'Target');
}

run();
