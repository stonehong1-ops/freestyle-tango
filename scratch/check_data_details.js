
const admin = require('firebase-admin');

const targetSa = {
  type: "service_account",
  project_id: "freestyle-tango-seoul",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@freestyle-tango-seoul.iam.gserviceaccount.com"
};

const targetApp = admin.initializeApp({
  credential: admin.credential.cert(targetSa)
}, 'target');

const targetDb = targetApp.firestore();

async function check() {
    console.log('--- Milonga Info ---');
    const milongaSnap = await targetDb.collection('milonga_info').get();
    milongaSnap.forEach(doc => {
        const data = doc.data();
        console.log(`Document [${doc.id}]:`);
        console.log(`  activeDate: ${data.activeDate}`);
        console.log(`  posterUrl: ${data.posterUrl}`);
    });

    console.log('\n--- Media (Lucy) ---');
    const mediaSnap = await targetDb.collection('media').where('type', '==', 'lucy').get();
    console.log(`Found ${mediaSnap.size} Media items of type 'lucy'`);
    mediaSnap.forEach(doc => {
        const data = doc.data();
        console.log(`Document [${doc.id}]:`);
        console.log(`  title: ${data.title}`);
        console.log(`  type: ${data.type}`);
        console.log(`  createdAt: ${data.createdAt}`);
    });
}

check().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
