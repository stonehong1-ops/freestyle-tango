const admin = require('firebase-admin');

// [SOURCE]
const sourceSa = {
  project_id: "tangostay-7355e",
  private_key: "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@tangostay-7355e.iam.gserviceaccount.com"
};
const sourceApp = admin.initializeApp({ credential: admin.credential.cert(sourceSa) }, 'source');

// [TARGET]
const targetSa = {
  project_id: "freestyle-tango-seoul",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@freestyle-tango-seoul.iam.gserviceaccount.com"
};
const targetApp = admin.initializeApp({ credential: admin.credential.cert(targetSa) }, 'target');

async function migrateAuth() {
  console.log('👥 Starting Auth Migration...');
  const result = await sourceApp.auth().listUsers(1000);
  console.log(`  - Found ${result.users.length} users. Migrating as many as possible...`);
  
  // Note: We can't migrate passwords without the hash key from the console.
  // We will migrate UIDs, Phone Numbers, and Display Names so data links stay intact.
  const usersToImport = result.users.map(u => ({
    uid: u.uid,
    email: u.email,
    phoneNumber: u.phoneNumber,
    displayName: u.displayName,
    photoURL: u.photoURL,
    disabled: u.disabled,
    metadata: u.metadata
  }));

  try {
    const importResult = await targetApp.auth().importUsers(usersToImport);
    console.log(`  ✅ Auth Import Finished: ${importResult.successCount} success, ${importResult.failureCount} failed.`);
    if (importResult.failureCount > 0) {
      console.log('  ⚠️ Note: Failure is often due to missing password hash. Users may need to sign in again via Phone/Social or reset password.');
    }
  } catch (e) {
    console.error('❌ Auth Import Fatal Error:', e);
  }
}

migrateAuth().then(() => process.exit(0));
