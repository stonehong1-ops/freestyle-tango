const admin = require('firebase-admin');

// [SOURCE] - US Project (tangostay-7355e)
const sourceSa = {
  type: "service_account",
  project_id: "tangostay-7355e",
  private_key: "-----BEGIN PRIVATE KEY-----\\ \BWCvSWbDEO4JIL6F1ST9XuDj1hQZiKrEH8ByABzrnqG32XxX8pTO+r9n3UAE5IHm\mmZxSC8+9QKBgFZPxIDkqB8r+YzD1ZWxuqTFWF60sTTcU8xpc0nExU9Mtrjm7yPM\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@tangostay-7355e.iam.gserviceaccount.com"
};
const sourceApp = admin.initializeApp({
  credential: admin.credential.cert(sourceSa)
}, 'source');

// [TARGET] - Seoul Project (freestyle-tango-seoul)
const targetSa = {
  type: "service_account",
  project_id: "freestyle-tango-seoul",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
  client_email: "firebase-adminsdk-fbsvc@freestyle-tango-seoul.iam.gserviceaccount.com"
};
const targetApp = admin.initializeApp({
  credential: admin.credential.cert(targetSa)
}, 'target');

const sourceDb = sourceApp.firestore();
const targetDb = targetApp.firestore();

const collectionsToMove = ['admin_settings', 'cron_logs'];

async function moveData() {
  console.log('🚀 Moving missing collections: US -> SEOUL');
  
  for (const collectionName of collectionsToMove) {
    console.log(`\n📦 Moving collection: [${collectionName}]`);
    const snapshot = await sourceDb.collection(collectionName).get();
    
    if (snapshot.empty) {
      console.log(`  - Collection is empty, skipping.`);
      continue;
    }

    console.log(`  - Found ${snapshot.size} documents. Copying...`);
    
    let batch = targetDb.batch();
    let count = 0;
    
    for (const doc of snapshot.docs) {
      const docRef = targetDb.collection(collectionName).doc(doc.id);
      batch.set(docRef, doc.data());
      count++;
      
      if (count % 450 === 0) {
        await batch.commit();
        batch = targetDb.batch();
        console.log(`    - Committed ${count} docs...`);
      }
    }
    
    await batch.commit();
    console.log(`  ✅ Done! Moved ${count} docs from ${collectionName}.`);
  }

  console.log('\n✨ All requested collections moved successfully.');
}

moveData()
  .then(() => {
    console.log('🎉 Task Finished!');
    process.exit(0);
  })
  .catch(err => {
    console.error('❌ Task Failed:', err);
    process.exit(1);
  });
