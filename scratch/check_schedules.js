...
async function checkSchedules() {
  const collections = ['extra_schedules', 'monthly_notices', 'tango_classes'];
  for (const col of collections) {
    const snap = await db.collection(col).limit(5).get();
    console.log(`Collection [${col}]: ${snap.size} docs found`);
    snap.forEach(d => console.log(` - ID: ${d.id}, Data:`, JSON.stringify(d.data()).substring(0, 100)));
  }
}

checkSchedules().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
